...
data{
  ...
  array[nsites] real fruit; // Number of ripe Opuntia fruits (standardised) 
...
parameters{
  ...
  real beta_fruit; // Effect of fruit
  ...
  real beta_grass; // Effect of grass
  real beta_forb; // Effect of forb
  real beta_shrub; // Effect of shrub
  real beta_succulent; // Effect of succulent
...
transformed parameters{
  ...
  vector[nsites] k; // Intercept term for each site
 ...
 // Gaussian process - non-centred
  ...
  for(isite in 1:nsites){
    // Occupancy submodel
    // Here Xmat[isite]*beta takes the 'isite'th row of Xmat and multiplies it by beta
    psi[isite] = inv_logit(k_bar + k[isite] + opuntia_vol[isite]*beta_opuntia + 
    fruit[isite]*beta_fruit +
    d_water[isite]*beta_d_water +
    d_road[isite]*beta_d_road + 
    livestock[isite]*beta_livestock +
    grass[isite]*beta_grass +
    forb[isite]*beta_forb +
    shrub[isite]*beta_forb +
    succulent[isite]*beta_succulent +
